...
* Importing `@Inject` and `@Injectable` annotations is now optional! It means that you don't have to write `use AcVendor\DI\Annotation\Inject` anymore
...
* The container is now auto-registered (as 'AcVendor\DI\Container'). You can now inject the container without registering it.
