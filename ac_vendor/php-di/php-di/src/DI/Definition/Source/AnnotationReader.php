...
use AcVendor\DI\Annotation\Inject;
use AcVendor\DI\Annotation\Injectable;
use AcVendor\DI\Definition\EntryReference;
use AcVendor\DI\Definition\Exception\AnnotationException;
use AcVendor\DI\Definition\Exception\DefinitionException;
use AcVendor\DI\Definition\ObjectDefinition;
use AcVendor\DI\Definition\ObjectDefinition\MethodInjection;
use AcVendor\DI\Definition\ObjectDefinition\PropertyInjection;
use Doctrine\Common\Annotations\AnnotationRegistry;
use Doctrine\Common\Annotations\Reader;
use Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use AcVendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationReader implements DefinitionSource
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'AcVendor\DI\Annotation\Inject');
        if ($annotation === null) {
            return null;
        }
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        ...
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'AcVendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            AnnotationRegistry::registerAutoloadNamespace('AcVendor\DI\Annotation', __DIR__ . '/../../../');
            $this->annotationReader = new SimpleAnnotationReader();
            $this->annotationReader->addNamespace('AcVendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()
                ->getClassAnnotation($class, 'AcVendor\DI\Annotation\Injectable');
